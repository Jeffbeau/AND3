MODULE restcom
!-
!$Header: /home/ioipsl/CVSROOT/IOIPSL/src/restcom.f90,v 2.5 2005/10/27 07:25:58 adm Exp $
!-
USE netcdf
!-
...
!-
  INTEGER,PARAMETER :: &
 &  max_var=200, max_file=50, max_dim=NF90_MAX_VAR_DIMS
!-
...
!-
  ...
  INTEGER,SAVE :: netcdf_id(max_file,2) = -1
!-
...
!-
  REAL,SAVE :: deltat(max_file), timeorig(max_file)
!-
...
!-
  INTEGER,SAVE :: tstp_out(max_file)
  INTEGER,SAVE :: itau_out(max_file)
!-
! Description of the axes in the output file
!-
...
!-
  INTEGER,SAVE :: &
 &  xax_infs(max_file,max_dim,2), yax_infs(max_file,max_dim,2),&
 &  zax_infs(max_file,max_dim,2), qax_infs(max_file,max_dim,2)
  INTEGER,SAVE :: &
 &  xax_nb(max_file)=0, yax_nb(max_file)=0, &
 &  zax_nb(max_file)=0, qax_nb(max_file)=0
!-
...
!-
 ...
 &  varname_in_length,varname_out_length, &
...
!-
!- Temporary variables in which we store the attributed
!- which are going to be given to
!- a new variable which is going to be defined.
!-
...
!-
  IF (l_fi) THEN
!---
    IF (check) WRITE(*,*) 'restini 1.0 : Open input file' 
!-- Add DOMAIN number and ".nc" suffix in file names if needed
    fname = fnamein
    CALL flio_dom_file (fname,domain_id)
!-- Open the file
    CALL restopenin &
      (nbfiles,fname,l_rw,iim,jjm, &
       lon,lat,llm,lev,ncfid)
    netcdf_id(nbfiles,1) = ncfid
!---
!-- 1.3 Extract the time information
!---
    CALL restsett (nbfiles,dt_tmp,date0_tmp)
...
!---
    tax_size_in(nbfiles) = 1
    IF ( .NOT.ALLOCATED(t_index) .AND. .NOT.ALLOCATED(t_julian) ) THEN
      IF (check) THEN
        WRITE(*,*) 'restini : Allocate times axes at :', &
                   max_file,tax_size_in(nbfiles)
      ENDIF
      ALLOCATE(t_index(max_file,tax_size_in(nbfiles)))
      t_index (:,:) = 0
      ALLOCATE(t_julian(max_file,tax_size_in(nbfiles)))
      t_julian (:,:) = 0.0
    ELSE
      IF (    (SIZE(t_index,DIM=2)  < tax_size_in(nbfiles)) &
          .OR.(SIZE(t_julian,DIM=2) < tax_size_in(nbfiles)) ) THEN
        IF (check) THEN
          WRITE(*,*) 'restini : Reallocate times axes at :', &
                     max_file,tax_size_in(nbfiles)
        ENDIF
        ALLOCATE(tmp_index(max_file,tax_size_in(nbfiles)))
        ALLOCATE(tmp_julian(max_file,tax_size_in(nbfiles)))
        tmp_index(:,:) = t_index(:,:)
        tmp_julian(:,:) = t_julian(:,:)
        DEALLOCATE(t_index)
        DEALLOCATE(t_julian)
    ...
    ENDIF
!---
    t_index(nbfiles,1) = itau
    t_julian(nbfiles,1) = date0
!---
  ENDIF
!-
...
!-- Open the file
    CALL restopenout &
      (nbfiles,fname,iim,jjm, &
       lon,lat,llm,lev,dt,date0,ncfid,domain_id)
    netcdf_id(nbfiles,2) = ncfid
  ELSE IF (l_fi.AND.l_fo) THEN
    netcdf_id(nbfiles,2) = netcdf_id(nbfiles,1)
    varname_out(nbfiles,:) = varname_in(nbfiles,:) 
    varname_out_length(nbfiles,:) = varname_in_length(nbfiles,:) 
    nbvar_out(nbfiles) = nbvar_in(nbfiles) 
    tind_varid_out(nbfiles) = tind_varid_in(nbfiles) 
    tax_varid_out(nbfiles) = tax_varid_in(nbfiles) 
    varid_out(nbfiles,:) = varid_in(nbfiles,:) 
    touched_out(nbfiles,:) = .TRUE.
  ENDIF
!-
...
!-
  ...
  ENDIF
!-
  IF (check) WRITE(*,*) 'After possible calendar configuration'
!-
...
! LOCAL
!-
  INTEGER :: var_dims(max_dim),dimlen(max_dim)
  INTEGER :: nb_dim,nb_var,id_unl,id,iv
  INTEGER :: iread,jread,lread,iret,idi
  INTEGER :: lon_vid,lat_vid
  REAL :: lon_read(iim,jjm),lat_read(iim,jjm)
  REAL :: lev_read(llm)
  REAL :: mdlon,mdlat
  CHARACTER(LEN=80) :: units,dimname
  LOGICAL :: check = .FALSE.
!---------------------------------------------------------------------
!-
...
!-
  ...
  ENDIF
!-
  IF (check) WRITE (*,*) "restopenin 0.0 ",TRIM(fname)
...
!-
  ...
  nbdim_in(fid) = nb_dim
  DO idi=1,nb_dim
    iret = NF90_INQUIRE_DIMENSION(ncfid,idi,len=dimlen(idi))
  ENDDO
  iread = dimlen(1)
  jread = dimlen(2)
  lread = dimlen(3)
...
!---
    IF (l_rw) THEN
      iret = NF90_INQUIRE_DIMENSION(ncfid,id_unl,len=tstp_out(fid))
      itau_out(fid) = -1
...
!-----
      DO id=1,nb_dim
        iret = NF90_INQUIRE_DIMENSION(ncfid,id,name=dimname)
        IF      (dimname(1:1) == 'x') THEN
          xax_nb(fid) = xax_nb(fid)+1
          iret = NF90_INQUIRE_DIMENSION &
 &                 (ncfid,id,len=xax_infs(fid,xax_nb(fid),1))
          xax_infs(fid,xax_nb(fid),2) = id
        ELSE IF (dimname(1:1) == 'y') THEN
          yax_nb(fid) = yax_nb(fid)+1
          iret = NF90_INQUIRE_DIMENSION &
 &                 (ncfid,id,len=yax_infs(fid,yax_nb(fid),1))
          yax_infs(fid,yax_nb(fid),2) = id
        ELSE IF (dimname(1:1) == 'z') THEN
          zax_nb(fid) = zax_nb(fid)+1
          iret = NF90_INQUIRE_DIMENSION &
 &                 (ncfid,id,len=zax_infs(fid,zax_nb(fid),1))
...
!-
 ...
 &    'to that of the model',' ')
  ENDIF
!-
! We know that we have a time axis. Thus the 4th dimension needs
! to be the levels.
!-
  IF (nb_dim > 3 .AND. dimlen(3) /= llm) THEN
    CALL ipslerr (3,'restopenin',&
 &    'The grid of the restart file does not correspond',&
 &    'to that of the model',' ')
  ENDIF
...
!---
 ...
 &             (ncfid,var_dims(id),len=vardims_in(fid,iv,id))
    ENDDO
!---
    varname_in_length(fid,iv) = LEN_TRIM(varname_in(fid,iv))
!---
...
!---
    IF (INDEX(units,'degrees_nort') >= 1) THEN
      lat_vid = iv
    ENDIF
    IF (INDEX(units,'degrees_east') >= 1) THEN
      lon_vid = iv
    ENDIF
!---
  ENDDO
...
!===
!-
SUBROUTINE restsett (fid,timestep,date0)
!---------------------------------------------------------------------
...
!-
!- What takes precedence ? No idea yet !
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  INTEGER :: fid
...
!-
  IF (.NOT.ALLOCATED(t_index) .AND. .NOT.ALLOCATED(t_julian) ) THEN
    IF (check) THEN
      WRITE(*,*) 'restsett : Allocate times axes at :', &
                 max_file,tax_size_in(fid)
    ENDIF
    ALLOCATE(t_index(max_file,tax_size_in(fid)))
    ALLOCATE(t_julian(max_file,tax_size_in(fid)))
  ELSE
    tszij = SIZE(t_index,DIM=2)
    IF (tszij < tax_size_in(fid)) THEN
      IF (check) THEN
        WRITE(*,*) 'restsett : Reallocate times axes at :', &
                   max_file,tax_size_in(fid)
      ENDIF
      ALLOCATE(tmp_index(max_file,tax_size_in(fid)))
      ALLOCATE(tmp_julian(max_file,tax_size_in(fid)))
...
!-
  IF (tax_size_in(fid) > 0 ) THEN
     calendar = 'XXXXX'
     iret = NF90_GET_ATT(ncfid,tax_varid_in(fid),'calendar',calendar)
     IF ( INDEX(calendar,'XXXXX') < 0 ) THEN
...
!-
  IF (tind_varid_in(fid) > 0) THEN
    iret = NF90_GET_VAR(ncfid,tind_varid_in(fid),t_index(fid,:))
    iret = NF90_GET_ATT(ncfid,tind_varid_in(fid),'units',itau_orig)
    itau_orig = &
      itau_orig(INDEX(itau_orig,'since')+6:LEN_TRIM(itau_orig))
    iret = NF90_GET_ATT &
 &           (ncfid,tind_varid_in(fid),'tstep_sec',timestep)
!---
!-- This time origine will dominate as it is linked to the time steps.
!---
...
!-
  IF (tax_varid_in(fid) > 0) THEN
    iret = NF90_GET_VAR(ncfid,tax_varid_in(fid),t_julian(fid,:))
    iret = NF90_GET_ATT(ncfid,tax_varid_in(fid),'units',tax_orig)
    tax_orig = tax_orig(INDEX(tax_orig,'since')+6:LEN_TRIM(tax_orig))
    iret = NF90_GET_ATT(ncfid,tax_varid_in(fid),'calendar',tmp_cal)
!---
    CALL strlowercase (tmp_cal)
    IF (INDEX(calend_str,tmp_cal) < 0) THEN
...
!---
    IF (check) WRITE(*,*) 'tax_orig : ',TRIM(tax_orig)
    ...
    t_julian(fid,:) = t_julian(fid,:)/un_jour+date0_ju
  ENDIF
!-
  IF (     (INDEX(itau_orig,'XXXXX') > 0) &
      .AND.(INDEX(tax_orig,'XXXXX')  < 0) ) THEN
!!- Compute the t_itau from the date read and the timestep in the input
  ENDIF
!-
  IF (     (INDEX(tax_orig,'XXXXX')  > 0) &
      .AND.(INDEX(itau_orig,'XXXXX') < 0) ) THEN
    DO it=1,tax_size_in(fid)
      t_julian(fid,it) = itau2date(t_index(fid,it),date0,timestep)
    ENDDO
  ENDIF
!-
...
!-
  IF ( (tax_varid_in(fid) < 0) .AND. (tind_varid_in(fid) < 0) ) THEN
    iax = -1
    DO iv=1,nbvar_in(fid)
      IF (INDEX(varname_in(fid,iv),'tsteps') > 0) THEN
        iax = iv
      ENDIF
    ENDDO
!---
...
!---
      CALL ymds2ju (year0,month0,day0,sec0,date0)
      t_julian(fid,1) = itau2date(t_index(fid,1),date0,timestep)
    ENDIF
  ENDIF
!----------------------
...
!-
  ...
  CHARACTER(LEN=30) :: timenow,conv
  LOGICAL :: check = .FALSE.
!---------------------------------------------------------------------
  IF (check) WRITE(*,*) "restopenout 0.0 ",TRIM(fname)
!-
...
!  iret = NF90_CREATE(fname,NF90_NOCLOBBER,ncfid)
  iret = NF90_CREATE(fname,NF90_CLOBBER,ncfid)
  IF (iret == -35) THEN
...
! 1.0 Longitude
!-
  IF (check) WRITE(*,*) "restopenout 1.0"
!-
  iret = NF90_DEF_VAR(ncfid,"nav_lon",NF90_FLOAT, &
 &                     (/ x_id, y_id /),nlonid)
...
! 2.0 Latitude
!-
  IF (check) WRITE(*,*) "restopenout 2.0"
!-
...
! 3.0 Levels
!-
  IF (check) WRITE(*,*) "restopenout 3.0"
!-
...
!-
  IF (check) WRITE(*,*) "restopenout 4.0"
!-
...
!-
  IF (check) WRITE(*,*) "restopenout 5.0"
!-
...
!-
  conv='GDT 1.2'
  iret = NF90_PUT_ATT(ncfid,NF90_GLOBAL,'Conventions',TRIM(conv))
...
!- 
  IF (check) WRITE(*,*) "restopenout END"
!-------------------------
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r1d : allocate buff_tmp = ",SIZE(var)
    ENDIF
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r1d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
!-
  IF (.NOT.ALLOCATED(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r1d : allocate buff_tmp2 = ",req_sz
    ENDIF
    ALLOCATE(buff_tmp2(req_sz))
  ELSE IF (req_sz > SIZE(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r1d : re-allocate buff_tmp2= ",req_sz
    ENDIF
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r2d : allocate buff_tmp = ",SIZE(var,1)
    ENDIF
    ALLOCATE(buff_tmp(SIZE(var,1)))
  ELSE IF (SIZE(var,1) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r2d : re-allocate buff_tmp= ",SIZE(var,1)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var,1)))
  ENDIF
!-
  IF (.NOT.ALLOCATED(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r2d : allocate buff_tmp2 = ",req_sz*jjm
    ENDIF
    ALLOCATE(buff_tmp2(req_sz*jjm))
  ELSE IF (req_sz*jjm > SIZE(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_opp_r2d : re-allocate buff_tmp2= ",req_sz*jjm
    ENDIF
    DEALLOCATE(buff_tmp2)
    ALLOCATE(buff_tmp2(req_sz*jjm))
  ENDIF
...
!---
 ...
 &      missing_val,nbindex,ijndex,scal,var_sz,buff_tmp)
!-----
      var(:,jj) = buff_tmp(:) 
    ENDDO
  ELSE
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) WRITE(*,*) "restget_r1d : allocate buff_tmp = ",SIZE(var)
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_r1d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
...
!-
  CALL restget_real &
    (fid,vname_q,iim,jjm,llm,itau,def_beha,buff_tmp)
!-
! 4.0 Transfer the buffer obtained from the restart file
!     into the variable the model expects
!-
...
!- LOCAL
!-
  INTEGER :: ji,jj,jl,req_sz,var_sz
  REAL,ALLOCATABLE,SAVE :: buff_tmp(:)
  CHARACTER(LEN=70) :: str,str2
  LOGICAL :: check = .FALSE.
!---------------------------------------------------------------------
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_r2d : allocate buff_tmp = ",SIZE(var)
    ENDIF
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_r2d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
...
!-
  ...
  IF (jjm > 0) req_sz = req_sz*jjm
  IF (llm > 0) req_sz = req_sz*llm
  var_sz = SIZE(var,2)*SIZE(var,1)
  IF (req_sz > var_sz) THEN
    WRITE(*,*) "RESGET_r2d :",vname_q
    WRITE(str, &
 &    '("Size of variable requested from file should be ",I6)') req_sz
    WRITE(str2, &
 &    '("but the provided variable can only hold ",I6)')  var_sz
...
! LOCAL
!-
  INTEGER :: ji,jj,jk,jl,req_sz,var_sz
  REAL,ALLOCATABLE,SAVE :: buff_tmp(:)
  CHARACTER(LEN=70) :: str,str2
  LOGICAL :: check = .FALSE.
!---------------------------------------------------------------------
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) WRITE(*,*) "restget_r3d : allocate buff_tmp = ",SIZE(var)
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restget_r3d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
...
!-
  ...
  IF (jjm > 0) req_sz = req_sz*jjm
  IF (llm > 0) req_sz = req_sz*llm
  var_sz = SIZE(var,3)*SIZE(var,2)*SIZE(var,1)
  IF (req_sz > var_sz) THEN
    WRITE(str, &
 &    '("Size of variable requested from file should be ",I6)') req_sz
    WRITE(str2, &
...
! LOCAL
!-
  INTEGER :: vid,vnb,ncfid
  INTEGER :: iret,index,it,ndim,ia
  CHARACTER(LEN=70) str,str2
...
!-
  CALL find_str &
    (nbvar_in(fid),varname_in(fid,1:nbvar_in(fid)), &
     varname_in_length(fid,1:nbvar_in(fid)),vname_q,vnb)
!-
...
!----
      ...
      varname_in_length(fid,vnb) = LEN_TRIM(vname_q)
...
SUBROUTINE restput_opp_r1d &
  (fid,vname_q,iim,jjm,llm,itau,var,OPERATOR,nbindex,ijndex)
!---------------------------------------------------------------------
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r1d : allocate buff_tmp = ",SIZE(var)
    ENDIF
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r1d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
!-
  IF (.NOT.ALLOCATED(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r1d : allocate buff_tmp2 = ",req_sz
    ENDIF
    ALLOCATE(buff_tmp2(req_sz))
  ELSE IF (req_sz > SIZE(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r1d : re-allocate buff_tmp2= ",req_sz
    ENDIF
    DEALLOCATE(buff_tmp2)
    ALLOCATE(buff_tmp2(req_sz))
  ENDIF
...
!---
    CALL mathop &
      (topp,SIZE(var),buff_tmp,missing_val,nbindex,ijndex, &
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r2d : allocate buff_tmp = ",SIZE(var,1)
    ENDIF
    ALLOCATE(buff_tmp(SIZE(var,1)))
  ELSE IF (SIZE(var,1) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r2d : re-allocate buff_tmp= ",SIZE(var,1)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var,1)))
  ENDIF
!-
  IF (.NOT.ALLOCATED(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r2d : allocate buff_tmp2 = ",req_sz*jjm
    ENDIF
    ALLOCATE(buff_tmp2(req_sz*jjm))
  ELSE IF (req_sz*jjm > SIZE(buff_tmp2)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_opp_r2d : re-allocate buff_tmp2= ",req_sz*jjm
    ENDIF
    DEALLOCATE(buff_tmp2)
    ALLOCATE(buff_tmp2(req_sz*jjm))
  ENDIF
...
!-----
      ist = (jj-1)*req_sz+1
      CALL mathop &
        (topp,var_sz,buff_tmp,missing_val,nbindex,ijndex, &
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_r1d : allocate buff_tmp = ",SIZE(var)
    ENDIF
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_r1d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
...
!-
  ...
  ENDDO
!-
  CALL restput_real (fid,vname_q,iim,jjm,llm,itau,buff_tmp)
!-------------------------
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) WRITE(*,*) "restput_r2d : allocate buff_tmp = ",SIZE(var)
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_r2d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
...
!-
  IF (.NOT.ALLOCATED(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_r3d : allocate buff_tmp = ",SIZE(var)
    ENDIF
    ALLOCATE(buff_tmp(SIZE(var)))
  ELSE IF (SIZE(var) > SIZE(buff_tmp)) THEN
    IF (check) THEN
      WRITE(*,*) "restput_r3d : re-allocate buff_tmp= ",SIZE(var)
    ENDIF
    DEALLOCATE(buff_tmp)
    ALLOCATE(buff_tmp(SIZE(var)))
  ENDIF
...
!-
  IF (check) WRITE(*,*) 'RESTPUT 1.0 : ',TRIM(vname_q)
!-
  CALL find_str &
    (nbvar_out(fid),varname_out(fid,1:nbvar_out(fid)), &
     varname_out_length(fid,1:nbvar_out(fid)),vname_q,vnb)
!-
  IF (check) THEN
...
!-
  IF (check) WRITE(*,*) 'RESTPUT 2.0 : ',vnb,vid
!-
...
!-
  IF (check) WRITE(*,*) 'RESTPUT 3.0 : ',itau,itau_out(fid)
!-
...
!-
  IF (iret /= NF90_NOERR) THEN
    WRITE (*,*) ' restput error ',NF90_STRERROR(iret)
  ENDIF
!-
...
!---------------------------------------------------------------------
  ...
  varname_out_length(fid,nbvar_out(fid)) = LEN_TRIM(varname)
!-
...
!---
  ELSE
!---
    IF (check) WRITE(*,*) 'restclo : Closing all files'
!---
    DO ifnc=1,nbfiles
