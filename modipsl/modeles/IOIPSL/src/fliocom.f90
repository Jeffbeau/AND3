!$Header: /home/ioipsl/CVSROOT/IOIPSL/src/fliocom.f90,v 2.32 2006/03/15 13:49:06 adm Exp $
!-
MODULE fliocom
!---------------------------------------------------------------------
USE netcdf
...
!!              If this argument is present with the value "REP"
...
!!
!!-- Rule 1 : we look for a correct "axis" attribute
!!
...
!!    t         T                         1
!!
!!-- Rule 2 : we look for a specific name
!!
!!  Axis       Names
!!
...
!!
!!-- Rule 3 : we look for a correct "units" attribute
!!
!!  Axis       Units
!!             (case insensitive)
!!
!!    x        'degree_e[...]'  'degrees_e[...]'
!!    y        'degree_n[...]'  'degrees_n[...]'
!!    z        'm[...]'         'km[...]'         'hpa[...]'
!!    t        'week[...]'      'day[...]'        'hour[...]'
!!             'minute[...]'    'second[...]'     'timesteps[...]'
!!
!!-- Rule 4 : we look for a variable with one dimension
...
! Check the mode
  IF (PRESENT(mode)) THEN
    IF ( (TRIM(MODE) == "REPLACE").OR.(TRIM(MODE) == "REP") ) THEN
      m_c = NF90_CLOBBER
    ELSE
      m_c = NF90_NOCLOBBER
    ENDIF
...
! Define "Conventions" global attribute
  i_rc = NF90_PUT_ATT(f_e,NF90_GLOBAL,'Conventions',"CF-1.0")
!-
...
!---
 ...
 &                      nw_di(k_1,f_i),levid)
    ...
    i_rc = NF90_PUT_ATT(f_e,levid,'standard_name','level')
...
!---------------------------------------------------------------------
  ixd = flio_qid(iid)
  IF (ixd < 0) THEN
    CALL ipslerr (3,TRIM(cpg),'Invalid internal file index.',' ',' ')
  ENDIF
!-----------------------
...
!---
    IF (i_v < 0) THEN
      ...
      CASE('x')
        n_r = 2
        c_r(1)='degree_e'; c_r(2)='degrees_e';
      CASE('y')
        n_r = 2
        c_r(1)='degree_n'; c_r(2)='degrees_n';
      CASE('z')
        n_r = 3
        c_r(1)='m'; c_r(2)='km'; c_r(3)='hpa';
      CASE('t')
        n_r = 6
        c_r(1)='week';   c_r(2)='day';    c_r(3)='hour';
        c_r(4)='minute'; c_r(5)='second'; c_r(6)='timesteps';
      END SELECT
!-----
      L_R3: DO kv=1,nw_nv(f_i)
        i_rc = NF90_GET_ATT(nw_id(f_i),kv,'units',c_t1)
        IF (i_rc == NF90_NOERR) THEN
          CALL strlowercase (c_t1)
          i_rc = NF90_INQUIRE_VARIABLE(nw_id(f_i),kv,ndims=n_d)
          IF (n_d <= l_d) THEN
            DO k=1,n_r
              IF (INDEX(c_t1,TRIM(c_r(k))) == 1) THEN
                i_v = kv; nbd = n_d;
                EXIT L_R3
              ENDIF
            ENDDO
...
!---
    IF (i_v < 0) THEN
      SELECT CASE (c_ax)
      CASE('x')
        dimnb = nw_di(nw_ai(k_lon,f_i),f_i)
      CASE('y')
        dimnb = nw_di(nw_ai(k_lat,f_i),f_i)
      CASE('z')
        dimnb = nw_di(nw_ai(k_lev,f_i),f_i)
      CASE('t')
        dimnb = nw_di(nw_ai(k_tim,f_i),f_i)
      END SELECT
!-----
