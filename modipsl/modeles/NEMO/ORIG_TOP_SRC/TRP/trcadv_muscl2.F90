MODULE trcadv_muscl2
   ...
   !! * Accessibility
   PUBLIC trc_adv_muscl2        ! routine called by trcstp.F90
...
#  include "passivetrc_substitute.h90"
   ...
   !! $Header: /home/opalod/NEMOCVSROOT/NEMO/TOP_SRC/TRP/trcadv_muscl2.F90,v 1.11 2006/04/10 15:38:54 opalod Exp $ 
   !! This software is governed by the CeCILL licence see modipsl/doc/NEMO_CeCILL.txt 
   !!----------------------------------------------------------------------

CONTAINS

   SUBROUTINE trc_adv_muscl2( kt )
      !!----------------------------------------------------------------------
      !!                   ***  ROUTINE trc_adv_muscl2  ***
      !!
      !! ** Purpose :   Compute the now trend due to total advection of passi-
      !!      ve tracer using a MUSCL scheme (Monotone Upstream-
      !!      Centered Scheme for Conservation Laws) and add it to the general
      !!      tracer trend.
      !!
      ...
      IF( kt == nittrc000 .AND. lwp ) THEN
         WRITE(numout,*)
         WRITE(numout,*) 'trc_adv_muscl2 : MUSCL2 advection scheme'
         WRITE(numout,*) '~~~~~~~~~~~~~~~'
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1      
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  zalpha = 0.5 - z0u
                  zu  = z0u - 0.5 * zun(ji,jj,jk) * rdttrc(jk) / e1u(ji,jj)
                  ...
                  zalpha = 0.5 - z0v
                  zv  = z0v - 0.5 * zvn(ji,jj,jk) * rdttrc(jk) / e2v(ji,jj)
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  IF( umask(ji,jj,jk) == 0. ) THEN
                     IF( zun(ji+1,jj,jk) > 0. .AND. ji /= jpi ) THEN
                        zt1(ji+1,jj,jk) = e2u(ji+1,jj)* fse3u(ji+1,jj,jk)   &
                           &            * zun(ji+1,jj,jk) * ( trb(ji+1,jj,jk,jn) + trb(ji+2,jj,jk,jn) ) * 0.5
                     ENDIF
                     IF( zun(ji-1,jj,jk) < 0. ) THEN
                        zt1(ji-1,jj,jk) = e2u(ji-1,jj)* fse3u(ji-1,jj,jk)   &
                           &            * zun(ji-1,jj,jk) * ( trb(ji-1,jj,jk,jn) + trb(ji  ,jj,jk,jn) ) * 0.5
                     ENDIF
                  ENDIF
                  IF( vmask(ji,jj,jk) == 0. ) THEN
                     IF( zvn(ji,jj+1,jk) > 0. .AND. jj /= jpj ) THEN
                        zt2(ji,jj+1,jk) = e1v(ji,jj+1) * fse3v(ji,jj+1,jk)   &
                           &            * zvn(ji,jj+1,jk) * ( trb(ji,jj+1,jk,jn) + trb(ji,jj+2,jk,jn) ) * 0.5
                     ENDIF
                     IF( zvn(ji,jj-1,jk) < 0. ) THEN
                        zt2(ji,jj-1,jk) = e1v(ji,jj-1)* fse3v(ji,jj-1,jk)   &
                           &            * zvn(ji,jj-1,jk) * ( trb(ji,jj-1,jk,jn) + trb(ji  ,jj,jk,jn) ) * 0.5
                     ENDIF
                  ENDIF
                  ...
                  IF( umask(ji,jj,jk) == 0. ) THEN
                     IF( zun(ji+1,jj,jk) > 0. .AND. ji /= jpi ) THEN
                        zt1(ji+1,jj,jk) = e2u(ji+1,jj) * zun(ji+1,jj,jk) * ( trb(ji+1,jj,jk,jn) + trb(ji+2,jj,jk,jn) ) * 0.5
                     ENDIF
                     IF( zun(ji-1,jj,jk) < 0. ) THEN
                        zt1(ji-1,jj,jk) = e2u(ji-1,jj) * zun(ji-1,jj,jk) * ( trb(ji-1,jj,jk,jn) + trb(ji  ,jj,jk,jn) ) * 0.5
                     ENDIF
                  ENDIF
                  IF( vmask(ji,jj,jk) == 0. ) THEN
                     IF( zvn(ji,jj+1,jk) > 0. .AND. jj /= jpj ) THEN
                        zt2(ji,jj+1,jk) = e1v(ji,jj+1) * zvn(ji,jj+1,jk) * ( trb(ji,jj+1,jk,jn) + trb(ji,jj+2,jk,jn) ) * 0.5
                     ENDIF
                     IF( zvn(ji,jj-1,jk) < 0. ) THEN
                        zt2(ji,jj-1,jk) = e1v(ji,jj-1) * zvn(ji,jj-1,jk) * ( trb(ji,jj-1,jk,jn) + trb(ji  ,jj,jk,jn) ) * 0.5
                     ENDIF
                  ENDIF
#endif
               END DO
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1      
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  zalpha = 0.5 + z0w
                  zw  = z0w - 0.5 * zwn(ji,jj,jk+1)* rdttrc(jk)/ fse3w(ji,jj,jk+1)
               ...
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  IF( tmask(ji,jj,jk+1) == 0. ) THEN
                     IF( zwn(ji,jj,jk) > 0. ) THEN
                        zt1(ji,jj,jk) = zwn(ji,jj,jk) * ( trb(ji,jj,jk-1,jn) + trb(ji,jj,jk,jn) ) * 0.5
                     ENDIF
                  ENDIF
               END DO
            END DO
...
CONTAINS
   SUBROUTINE trc_adv_muscl2( kt ) 
      ...
      WRITE(*,*) 'trc_adv_muscl2: You should not have seen this print! error?', kt
   END SUBROUTINE trc_adv_muscl2
#endif

   !!======================================================================
END MODULE trcadv_muscl2
