...
#  include "domzgr_substitute.h90"
   ...
   !! $Header: /home/opalod/NEMOCVSROOT/NEMO/OPA_SRC/mppini.F90,v 1.9 2006/04/10 15:46:05 opalod Exp $ 
...
CONTAINS
   ...
   SUBROUTINE mpp_init
      ...
      IF(  jpni /= 1 .OR. jpnj /= 1 .OR. jpnij /= 1 ) THEN
          IF(lwp)WRITE(numout,cform_err)
          IF(lwp)WRITE(numout,*) 'equality  jpni = jpnj = jpnij = 1 is not satisfied'
          IF(lwp)WRITE(numout,*) 'the domain is lay out for distributed memory computing! '
          nstop = nstop + 1
      ENDIF
   ...
   !!   'key_mpp_shmem'                 SHMEM massively parallel processing
   !!----------------------------------------------------------------------
 ...
 9202    FORMAT(' ',i3,' *  ',20(i3,'  x',i3,'   *   '))
     ...
     ! Save processor layout in layout.dat file 
      ...
      IF( nperio == 1 .AND. jpni /= 1 )THEN
         IF(lwp) WRITE(numout,cform_err)
         IF(lwp) WRITE(numout,*) ' mpp_init: error on cyclicity'
         nstop = nstop + 1
      ENDIF
...
#  include "mppini_2.h90"
   ...
   SUBROUTINE mpp_init_ioipsl
      ...
      ! The domain is splitted only horizontally along i- or/and j- direction
      ...
      inbondj = -1                                   ! general case
      IF( narea   >  jpni          )   inbondj = 0      ! first row of processor
      IF( narea   >  (jpnj-1)*jpni )   inbondj = 1      ! last  row of processor
      IF( jpnj    == 1             )   inbondj = 2      ! one processor only in j-direction
      IF( inbondj == -1 .OR. inbondj == 2 )   ihals(2) = 0
      IF( inbondj ==  1 .OR. inbondj == 2 )   ihale(2) = 0
         
      inbondi = 0                                    ! 
      IF( MOD( narea, jpni ) == 1 )   inbondi = -1      !
      IF( MOD( narea, jpni ) == 0 )   inbondi =  1      !
      IF( jpni               == 1 )   inbondi =  2      ! one processor only in i-direction
      IF( inbondi == -1 .OR. inbondi == 2 )   ihals(1) = 0
      IF( inbondi ==  1 .OR. inbondi == 2 )   ihale(1) = 0
