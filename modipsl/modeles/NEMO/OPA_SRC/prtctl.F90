MODULE prtctl
   ...
   !! $Header: /home/opalod/NEMOCVSROOT/NEMO/OPA_SRC/prtctl.F90,v 1.2 2006/03/21 15:54:15 opalod Exp $ 
...
CONTAINS
   ...
   SUBROUTINE prt_ctl (tab2d_1, tab3d_1, mask1, clinfo1, tab2d_2, tab3d_2, mask2, clinfo2, ovlap, kdim, clinfo3)
      ...
      !! * Local declarations
      INTEGER :: overlap, numid, jn, sind, eind, kdir
      ...
      IF( PRESENT(tab3d_1) )  ztab3d_1(:,:,:)= tab3d_1(:,:,:)
      IF( PRESENT(tab3d_2) )  ztab3d_2(:,:,:)= tab3d_2(:,:,:)
      ...
      DO jn = sind, eind
         ...
         ! Print the result
         IF( PRESENT(clinfo2) .OR. PRESENT(tab2d_2) .OR. PRESENT(tab3d_2) )   THEN
            WRITE(numid,FMT='(a,D23.16,3x,a,D23.16)')clinfo1, zsum1-zvctl1, cl2, zsum2-zvctl2
             ...
             ENDIF
         ELSE
            WRITE(numid,FMT='(a,D23.16)')clinfo1, zsum1
         ENDIF
   ...
   SUBROUTINE prt_ctl_info (clinfo1, ivar1, clinfo2, ivar2)
      !!----------------------------------------------------------------------
   ...
   SUBROUTINE prt_ctl_init
      ...
      DO jn = sind, eind
         ...
         ENDIF
         WRITE(numid,*) 
         WRITE(numid,*) 'prt_ctl :  Sum control indices'
         WRITE(numid,*) '~~~~~~~'
         WRITE(numid,*)
         WRITE(numid,9000)'                                nlej   = ', nlejtl(jn), '              '
         WRITE(numid,9000)'                  ------------- njctle = ', njctle, ' -------------'
         WRITE(numid,9001)'                  |                                       |'
         WRITE(numid,9001)'                  |                                       |'
         WRITE(numid,9001)'                  |                                       |'
         WRITE(numid,9002)'           nictls = ', nictls,  '                           nictle = ', nictle
         WRITE(numid,9002)'           nldi   = ', nlditl(jn),  '                           nlei   = ', nleitl(jn)
         WRITE(numid,9001)'                  |                                       |'
         WRITE(numid,9001)'                  |                                       |'
         WRITE(numid,9001)'                  |                                       |'
         WRITE(numid,9004)'  njmpp  = ',njmpptl(jn),'   ------------- njctls = ', njctls, ' -------------'
         WRITE(numid,9003)'           nimpp  = ', nimpptl(jn), '        nldj   = ', nldjtl(jn), '              '
         WRITE(numid,*)
         WRITE(numid,*)
