!!DB 2008.03.13 
!!DBG-TEST: Change output to a .dat file written by lwp
...
!!DB
            ...
            if(lwp) then
               open(555,file='db.float_traject.info')
               write(555,*)'Run name  #-floats nwritefl: ', cexper,jpnfl,nwritefl
               write(555,*)'Columns in ',clname,': float-# kt i j k lon lat z niso ngrp'
               close(555)
            endif
...
!            WRITE(numflo) flxx,flyy,flzz,nisobfl,ngrpfl,ztemp,zsal, FLOAT(ndastp)
            if(lwp) then
               do jfl = 1, jpnfl
                  ...
                  ibfl  = INT ( tpjfl(jfl) )
                  icfl  = INT ( tpkfl(jfl) )
                  write(numflo,'(5(i6,2x),3(e12.6,1x),2(i3,1x))')   &
...
!!DB move inside lk_mpp if-statement
!         WRITE(numflo) flxx,flyy,flzz,nisobfl,ngrpfl,ztemp,zsal, FLOAT(ndastp)
