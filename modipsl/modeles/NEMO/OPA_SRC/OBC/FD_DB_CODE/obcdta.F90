!!DB: 2008.10.08
!!Eliminate calls to IOIPSL ... in progress ...
        ...
        ELSE
           ...
           IF( lp_obc_east )   THEN
              ...
              IF( l_exv )   THEN
                 itobce = f_d(1)
              ELSE
                 WRITE(numout,*) ' Variable ',TRIM(v_name),' not found in file ','obceast_TS.nc'
              ENDIF
           ENDIF
           IF( lp_obc_west )   THEN
              ...
              IF( l_exv )   THEN
                 itobcw = f_d(1)
              ELSE
                 WRITE(numout,*) ' Variable ',TRIM(v_name),' not found in file ','obcwest_TS.nc'
              ENDIF
           ENDIF
           IF( lp_obc_north )   THEN
              ...
              IF( l_exv )   THEN
                 itobcn = f_d(1)
              ELSE
                 WRITE(numout,*) ' Variable ',TRIM(v_name),' not found in file ','obcnorth_TS.nc'
              ENDIF
           ENDIF
           IF( lp_obc_south )   THEN
              ...
              IF( l_exv )   THEN
                 itobcs = f_d(1)
              ELSE
                 WRITE(numout,*) ' Variable ',TRIM(v_name),' not found in file ','obcsouth_TS.nc'
              ENDIF
           ENDIF
...
!sujie
     ...
     IF( ( nobc_dta == 1 ) .AND. ( ( kt == nit000 .AND. nlecto == 0 ) .OR. itimo  /= ntobc1 ) )   THEN
        ...
        IF( lp_obc_east )   THEN
           ...
           CALL flioopfd ('obceast_TS.nc',fid_e)
!DB 
           CALL obc_dta_gv (fid_e,'y','vosaline',jpjef-jpjed+1,ntobc1,pdta_4D=sedta(jpjed:jpjef,:,1:5,1))
           CALL obc_dta_gv (fid_e,'y','vosaline',jpjef-jpjed+1,ntobc2,pdta_4D=sedta(jpjed:jpjef,:,1:5,2))
           CALL obc_dta_gv (fid_e,'y','votemper',jpjef-jpjed+1,ntobc1,pdta_4D=tedta(jpjed:jpjef,:,1:5,1))
           CALL obc_dta_gv (fid_e,'y','votemper',jpjef-jpjed+1,ntobc2,pdta_4D=tedta(jpjed:jpjef,:,1:5,2))	   

           CALL flioclo (fid_e)

           CALL flioopfd ('obceast_U.nc',fid_e)
!!DB
           CALL obc_dta_gv (fid_e,'y','vozocrtx',jpjef-jpjed+1,ntobc1,pdta_3D=uedta(jpjed:jpjef,:,1))
           CALL obc_dta_gv (fid_e,'y','vozocrtx',jpjef-jpjed+1,ntobc1,pdta_3D=uedta(jpjed:jpjef,:,2))
           CALL flioclo (fid_e)
           ...
           IF( lwp .AND.  ( kt == nit000 .OR. nprint /= 0 ) )   THEN
              ...
              CALL prihre( tedta(:,:,1,1),jpjef-jpjed+1,jpk,1,jpjef-jpjed+1,ikprint, &
                 &         jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Salinity  record 1 - printout every 3 level'
              CALL prihre( sedta(:,:,1,1), jpjef-jpjed+1, jpk, 1, jpjef-jpjed+1, ikprint, &
                 &        jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Normal velocity U  record 1  - printout every 3 level'
              CALL prihre( uedta(:,:,1), jpjef-jpjed+1, jpk, 1, jpjef-jpjed+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
           ENDIF
        ENDIF
        ...
        IF( lp_obc_west )   THEN
           ...
           CALL flioopfd ('obcwest_TS.nc',fid_w)
!DB
           CALL obc_dta_gv (fid_w,'y','vosaline',jpjwf-jpjwd+1,ntobc1,pdta_4D=swdta(jpjwd:jpjwf,:,1:5,1))
           CALL obc_dta_gv (fid_w,'y','vosaline',jpjwf-jpjwd+1,ntobc2,pdta_4D=swdta(jpjwd:jpjwf,:,1:5,2))
           CALL obc_dta_gv (fid_w,'y','votemper',jpjwf-jpjwd+1,ntobc1,pdta_4D=twdta(jpjwd:jpjwf,:,1:5,1))
           CALL obc_dta_gv (fid_w,'y','votemper',jpjwf-jpjwd+1,ntobc2,pdta_4D=twdta(jpjwd:jpjwf,:,1:5,2))
           CALL flioclo (fid_w)

           CALL flioopfd ('obcwest_U.nc',fid_w)
!!DB
           CALL obc_dta_gv (fid_w,'y','vozocrtx',jpjwf-jpjwd+1,ntobc1,pdta_3D=uwdta(jpjwd:jpjwf,:,1))
           CALL obc_dta_gv (fid_w,'y','vozocrtx',jpjwf-jpjwd+1,ntobc2,pdta_3D=uwdta(jpjwd:jpjwf,:,2))
           CALL flioclo (fid_w)
           ...
           IF( lwp .AND.  ( kt == nit000 .OR. nprint /= 0 ) )   THEN
              ...
              WRITE(numout,*) ' Temperature  record 1 - printout every 3 level'
              CALL prihre( twdta(:,:,1,1), jpjwf-jpjwd+1, jpk, 1, jpjwf-jpjwd+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Salinity  record 1 - printout every 3 level'
              CALL prihre( swdta(:,:,1,1), jpjwf-jpjwd+1, jpk, 1, jpjwf-jpjwd+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Normal velocity U  record 1  - printout every 3 level'
              CALL prihre( uwdta(:,:,1), jpjwf-jpjwd+1, jpk, 1, jpjwf-jpjwd+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
           ENDIF
        ENDIF
        ...
        IF( lp_obc_north )   THEN
           CALL flioopfd ('obcnorth_TS.nc',fid_n)
!!DB
           CALL obc_dta_gv (fid_n,'x','vosaline',jpinf-jpind+1,ntobc1,pdta_4D=sndta(jpind:jpinf,:,1:5,1))
           CALL obc_dta_gv (fid_n,'x','vosaline',jpinf-jpind+1,ntobc2,pdta_4D=sndta(jpind:jpinf,:,1:5,2))
           CALL obc_dta_gv (fid_n,'x','votemper',jpinf-jpind+1,ntobc1,pdta_4D=tndta(jpind:jpinf,:,1:5,1))
           CALL obc_dta_gv (fid_n,'x','votemper',jpinf-jpind+1,ntobc2,pdta_4D=tndta(jpind:jpinf,:,1:5,2))
           CALL flioclo (fid_n)

           CALL flioopfd ('obcnorth_V.nc',fid_n)
           CALL obc_dta_gv (fid_n,'x','vomecrty',jpinf-jpind+1,ntobc1,pdta_3D=vndta(jpind:jpinf,:,1))
           CALL obc_dta_gv (fid_n,'x','vomecrty',jpinf-jpind+1,ntobc2,pdta_3D=vndta(jpind:jpinf,:,2))
           CALL flioclo (fid_n)
           ...
           IF( lwp .AND.  ( kt == nit000 .OR. nprint /= 0 ) )   THEN
              ...
              WRITE(numout,*) ' Temperature  record 1 - printout every 3 level'
              CALL prihre( tndta(:,:,1,1), jpinf-jpind+1, jpk, 1, jpinf-jpind+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Salinity  record 1 - printout every 3 level'
              CALL prihre( sndta(:,:,1,1), jpinf-jpind+1, jpk, 1, jpinf-jpind+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Normal velocity V  record 1  - printout every 3 level'
              CALL prihre( vndta(:,:,1), jpinf-jpind+1, jpk, 1, jpinf-jpind+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
           ENDIF
        ENDIF
        ...
        IF( lp_obc_south )   THEN
           CALL flioopfd ('obcsouth_TS.nc',fid_s)
!!DB
           CALL obc_dta_gv (fid_s,'x','vosaline',jpisf-jpisd+1,ntobc1,pdta_4D=ssdta(jpisd:jpisf,:,1:5,1))
           CALL obc_dta_gv (fid_s,'x','vosaline',jpisf-jpisd+1,ntobc2,pdta_4D=ssdta(jpisd:jpisf,:,1:5,2))
           CALL obc_dta_gv (fid_s,'x','votemper',jpisf-jpisd+1,ntobc1,pdta_4D=tsdta(jpisd:jpisf,:,1:5,1))
           CALL obc_dta_gv (fid_s,'x','votemper',jpisf-jpisd+1,ntobc2,pdta_4D=tsdta(jpisd:jpisf,:,1:5,2))
           CALL flioclo (fid_s)

           CALL flioopfd ('obcsouth_V.nc',fid_s)
!!DB
           CALL obc_dta_gv (fid_s,'x','vomecrty',jpisf-jpisd+1,ntobc1,pdta_3D=vsdta(jpisd:jpisf,:,1))
           CALL obc_dta_gv (fid_s,'x','vomecrty',jpisf-jpisd+1,ntobc2,pdta_3D=vsdta(jpisd:jpisf,:,2))
           CALL flioclo (fid_s)
           ...
           IF( lwp .AND.  ( kt == nit000 .OR. nprint /= 0 ) )   THEN
              ...
              WRITE(numout,*) ' Temperature  record 1 - printout every 3 level'
              CALL prihre( tsdta(:,:,1,1), jpisf-jpisd+1, jpk, 1, jpisf-jpisd+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Salinity  record 1 - printout every 3 level'
              CALL prihre( ssdta(:,:,1,1), jpisf-jpisd+1, jpk, 1, jpisf-jpisd+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
              WRITE(numout,*)
              WRITE(numout,*) ' Normal velocity V  record 1  - printout every 3 level'
              CALL prihre( vsdta(:,:,1), jpisf-jpisd+1, jpk, 1, jpisf-jpisd+1, ikprint, &
                 &         jpk, 1, -3, 1., numout )
           ENDIF
        ENDIF
...
!!DBG
        if(kt == nit000) write(numout,*)'IN lp_obc_east: ADJUSTING TRANSPORT, narea= ',narea
...
!!DB 2007.11.26
        ...
        if(lwp .AND. kt-nit000 == 0) write(numout,*)'DBG: (obcdta, dt=1) nday_year, fac_SBI = ', &
                    nday_year,fac_SBI
...
!DB  East Inflow: Shelf break region ---------------------------------
        ...
        if(lwp .AND. kt-nit000 == 0) &
             & write(numout,*)'DBG: (obcdta, dt=1) nday_year, fac_SB_east = ', nday_year, fac_SB_east
        if(lwp .AND. mod(kt-nit000,int(86400/rdt)) == 0)write(numout,'(A51,2x,2(i5,1x),2(f8.3,1x))') &
...
!!DBG: check above calcs
           if(mod(kt-nit000,int(86400/rdt)) == 0) then
              write(numout,'(A77,2x,2(i5,1x),4(f8.3,1x))') &
...
!!DBG
        if(kt==nit000) then
           write(580+narea,*)nday,i15,zxy
           do jk = 1, jpkm1
              do jj = njw0p1, njw1m1
                 ij = jj -1 + njmpp
                 write(580+narea,'(3(i5,1x),5(e10.4,1x))')jk,jj,ij,twdta(ij,jk,1,1),twdta(ij,jk,1,2), & 
                      tfow(jj,jk,1)
              enddo
           enddo
        endif
...
!!DBG
        if(kt == nit000) write(numout,*)'IN lp_obc_west ADJUSTING TRANSPORT, narea= ',narea

!DB  west Inflow---------------------------------
!!DBG
        if(lwp .AND. kt-nit000 == 0) &
             &  write(numout,*)'DBG: (obcdta, dt=1) nday_year, fac_SB_west = fac_SB_east = ',nday_year, fac_SB_east
...
!        if(kt == nit000) write(numout,*)'IN lp_obc_south ADJUSTING TRANSPORT'
...
!!DB -- Hardwired choice of # of tidal constituents
!     nntide =5
     nntide =1
...
!ylu
           ...
           tidevbtemag(:,:)=0.0
           tidevbtepha(:,:)=0.0
           CALL flioopfd ('obceast_tide.nc',fid_e)
           do ntide = 1, nntide
!!DB
              CALL obc_dta_gv (fid_e,'y','tidesshmag',jpjef-jpjed+1,ntide,pdta_2D=tidesshemag(jpjed:jpjef,ntide))
              CALL obc_dta_gv (fid_e,'y','tidesshpha',jpjef-jpjed+1,ntide,pdta_2D=tidesshepha(jpjed:jpjef,ntide))
              CALL obc_dta_gv (fid_e,'y','tidevbtmag',jpjef-jpjed+1,ntide,pdta_2D=tidevbtemag(jpjed:jpjef,ntide))
              CALL obc_dta_gv (fid_e,'y','tidevbtpha',jpjef-jpjed+1,ntide,pdta_2D=tidevbtepha(jpjed:jpjef,ntide))
           end do
           CALL flioclo (fid_e)
         ENDIF
...
!ylu
           ...
           tidevbtwmag(:,:)=0.0
           tidevbtwpha(:,:)=0.0
           CALL flioopfd ('obcwest_tide.nc',fid_w)
           do ntide = 1, nntide
!!DB
              CALL obc_dta_gv (fid_w,'y','tidesshmag',jpjwf-jpjwd+1,ntide,pdta_2D=tidesshwmag(jpjwd:jpjwf,ntide))
              CALL obc_dta_gv (fid_w,'y','tidesshpha',jpjwf-jpjwd+1,ntide,pdta_2D=tidesshwpha(jpjwd:jpjwf,ntide))
              CALL obc_dta_gv (fid_w,'y','tidevbtmag',jpjwf-jpjwd+1,ntide,pdta_2D=tidevbtwmag(jpjwd:jpjwf,ntide))
              CALL obc_dta_gv (fid_w,'y','tidevbtpha',jpjwf-jpjwd+1,ntide,pdta_2D=tidevbtwpha(jpjwd:jpjwf,ntide))
           end do
           CALL flioclo (fid_w)
         ENDIF
...
!ylu
           ...
           tidevbtnmag(:,:)=0.0
           tidevbtnpha(:,:)=0.0
           CALL flioopfd ('obcnorth_tide.nc',fid_n)
           do ntide = 1, nntide
!!DB
              CALL obc_dta_gv (fid_n,'x','tidesshmag',jpinf-jpind+1,ntide,pdta_2D=tidesshnmag(jpind:jpinf,ntide))
              CALL obc_dta_gv (fid_n,'x','tidesshpha',jpinf-jpind+1,ntide,pdta_2D=tidesshnpha(jpind:jpinf,ntide))
              CALL obc_dta_gv (fid_n,'x','tidevbtmag',jpinf-jpind+1,ntide,pdta_2D=tidevbtnmag(jpind:jpinf,ntide))
              CALL obc_dta_gv (fid_n,'x','tidevbtpha',jpinf-jpind+1,ntide,pdta_2D=tidevbtnpha(jpind:jpinf,ntide))
           end do 
           CALL flioclo (fid_n)
         ENDIF
...
!ylu
           ...
           tidevbtsmag(:,:)=0.0
           tidevbtspha(:,:)=0.0
           CALL flioopfd ('obcsouth_tide.nc',fid_s)
           do ntide = 1, nntide
!!DB
              CALL obc_dta_gv (fid_s,'x','tidesshmag',jpisf-jpisd+1,ntide,pdta_2D=tidesshsmag(jpisd:jpisf,ntide))
              CALL obc_dta_gv (fid_s,'x','tidesshpha',jpisf-jpisd+1,ntide,pdta_2D=tidesshspha(jpisd:jpisf,ntide))
              CALL obc_dta_gv (fid_s,'x','tidevbtmag',jpisf-jpisd+1,ntide,pdta_2D=tidevbtsmag(jpisd:jpisf,ntide))
              CALL obc_dta_gv (fid_s,'x','tidevbtpha',jpisf-jpisd+1,ntide,pdta_2D=tidevbtspha(jpisd:jpisf,ntide))
           end do
           CALL flioclo (fid_s)
         ENDIF
...
! ------------------
        ...
        IF( lp_obc_east ) THEN
           ...
           CALL flioopfd ('obceast_TS.nc',fid_e)
!!DB
           CALL obc_dta_gv (fid_e,'y','vossurfh',jpjef-jpjed+1,ntobc1,pdta_2D=sshedta(jpjed:jpjef,1))
           CALL obc_dta_gv (fid_e,'y','vossurfh',jpjef-jpjed+1,ntobc2,pdta_2D=sshedta(jpjed:jpjef,2))
           IF( lk_dynspg_ts ) THEN
              CALL obc_dta_gv (fid_e,'y','vossurfh',jpjef-jpjed+1,ntobc3,pdta_2D=sshedta(jpjed:jpjef,3))
           ENDIF
           CALL flioclo (fid_e)

           CALL flioopfd ('obceast_U.nc',fid_e)
!!DB
           CALL obc_dta_gv (fid_e,'y','vozoubt',jpjef-jpjed+1,ntobc1,pdta_2D=ubtedta(jpjed:jpjef,1))
           CALL obc_dta_gv (fid_e,'y','vozoubt',jpjef-jpjed+1,ntobc2,pdta_2D=ubtedta(jpjed:jpjef,2))
           IF( lk_dynspg_ts ) THEN
              CALL obc_dta_gv (fid_e,'y','vozoubt',jpjef-jpjed+1,ntobc3,pdta_2D=ubtedta(jpjed:jpjef,3))
           ENDIF
           CALL flioclo (fid_e)
        ...
        IF( lp_obc_west ) THEN
           ...
           CALL flioopfd ('obcwest_TS.nc',fid_w)
!!DB
           CALL obc_dta_gv (fid_w,'y','vossurfh',jpjwf-jpjwd+1,ntobc1,pdta_2D=sshwdta(jpjwd:jpjwf,1))
           CALL obc_dta_gv (fid_w,'y','vossurfh',jpjwf-jpjwd+1,ntobc2,pdta_2D=sshwdta(jpjwd:jpjwf,2))
           IF( lk_dynspg_ts ) THEN
              CALL obc_dta_gv (fid_w,'y','vossurfh',jpjwf-jpjwd+1,ntobc3,pdta_2D=sshwdta(jpjwd:jpjwf,3))
           ENDIF
           CALL flioclo (fid_w)

           CALL flioopfd ('obcwest_U.nc',fid_w)
           CALL obc_dta_gv (fid_w,'y','vozoubt',jpjwf-jpjwd+1,ntobc1,pdta_2D=ubtwdta(jpjwd:jpjwf,1))
           CALL obc_dta_gv (fid_w,'y','vozoubt',jpjwf-jpjwd+1,ntobc2,pdta_2D=ubtwdta(jpjwd:jpjwf,2))
           IF( lk_dynspg_ts ) THEN
              CALL obc_dta_gv (fid_w,'y','vozoubt',jpjwf-jpjwd+1,ntobc3,pdta_2D=ubtwdta(jpjwd:jpjwf,3))
           ENDIF
           CALL flioclo (fid_w)
        ...
        IF( lp_obc_north) THEN
           ...
           CALL flioopfd ('obcnorth_TS.nc',fid_n)
!!DB
           CALL obc_dta_gv (fid_n,'x','vossurfh',jpinf-jpind+1,ntobc1,pdta_2D=sshndta(jpind:jpinf,1))
           CALL obc_dta_gv (fid_n,'x','vossurfh',jpinf-jpind+1,ntobc2,pdta_2D=sshndta(jpind:jpinf,2))
           IF( lk_dynspg_ts ) THEN
               CALL obc_dta_gv (fid_n,'x','vossurfh',jpinf-jpind+1,ntobc3,pdta_2D=sshndta(jpind:jpinf,3))
           ENDIF
           CALL flioclo (fid_n)

           CALL flioopfd ('obcnorth_V.nc',fid_n)
           CALL obc_dta_gv (fid_n,'x','vomevbt',jpinf-jpind+1,ntobc1,pdta_2D=vbtndta(jpind:jpinf,1))
           CALL obc_dta_gv (fid_n,'x','vomevbt',jpinf-jpind+1,ntobc2,pdta_2D=vbtndta(jpind:jpinf,2))
           IF( lk_dynspg_ts ) THEN
            CALL obc_dta_gv (fid_n,'x','vomevbt',jpinf-jpind+1,ntobc3,pdta_2D=vbtndta(jpind:jpinf,3))
           ENDIF
           CALL flioclo (fid_n)
        ...
        IF( lp_obc_south) THEN
           ...
           CALL flioopfd ('obcsouth_TS.nc',fid_s)
!!DB
           CALL obc_dta_gv (fid_s,'x','vossurfh',jpisf-jpisd+1,ntobc1,pdta_2D=sshsdta(jpisd:jpisf,1))
           CALL obc_dta_gv (fid_s,'x','vossurfh',jpisf-jpisd+1,ntobc2,pdta_2D=sshsdta(jpisd:jpisf,2))
           IF( lk_dynspg_ts ) THEN
               CALL obc_dta_gv (fid_s,'x','vossurfh',jpisf-jpisd+1,ntobc3,pdta_2D=sshsdta(jpisd:jpisf,3))
           ENDIF
           CALL flioclo (fid_s)

           CALL flioopfd ('obcsouth_V.nc',fid_s)
           CALL obc_dta_gv (fid_s,'x','vomevbt',jpisf-jpisd+1,ntobc1,pdta_2D=vbtsdta(jpisd:jpisf,1))
           CALL obc_dta_gv (fid_s,'x','vomevbt',jpisf-jpisd+1,ntobc2,pdta_2D=vbtsdta(jpisd:jpisf,2))
           IF( lk_dynspg_ts ) THEN
               CALL obc_dta_gv (fid_s,'x','vomevbt',jpisf-jpisd+1,ntobc3,pdta_2D=vbtsdta(jpisd:jpisf,3))
           ENDIF
           CALL flioclo (fid_s)
...
!   SUBROUTINE obc_dta_gv (ifid,cldim,clobc,kobcij,ktobc,pdta_2D,pdta_3D)
  SUBROUTINE obc_dta_gv (ifid,cldim,clobc,kobcij,ktobc,pdta_2D,pdta_3D,pdta_4D)
     !!-----------------------------------------------------------------------------
...
!sujie
     ...
     CALL flioinqv (ifid,TRIM(clobc),l_exv,nb_dims=indim,len_dims=f_d)
     IF( l_exv )   THEN
...
!sujie
           ...
           CALL fliogetv (ifid,TRIM(clobc),pdta_4D,start=istart(1:4),count=icount(1:4))
        ELSE
           IF( lwp )   THEN
        ...
        ENDIF
     ELSE
        WRITE(numout,*) ' Variable ',TRIM(clobc),' not found'
     ENDIF
