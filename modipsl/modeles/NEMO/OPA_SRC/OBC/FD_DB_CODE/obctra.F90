   ...
   SUBROUTINE obc_tra_east ( kt )
            ...
            DO jk = 1, jpkm1
               DO jj = nje0, nje1
                  ta(ji,jj,jk) = ta(ji,jj,jk) * (1. - temsk(jj,jk)) + &
                                 tfoe(jj,jk,1)*temsk(jj,jk)
                  sa(ji,jj,jk) = sa(ji,jj,jk) * (1. - temsk(jj,jk)) + &
                                 sfoe(jj,jk,1)*temsk(jj,jk)
               END DO
            END DO
         END DO
      ...
      ! -----------------------------------------------------
         ...
         DO ji = fs_nie0+1, fs_nie1+1 ! Vector opt.
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1
!FD change for an advection characteristic + relaxation
!FD      ! ... i-phase speed ratio (from averaged of v_cxebnd)
!FD               z05cx = ( 0.5 * ( v_cxebnd(jj,jk) + v_cxebnd(jj-1,jk) ) ) / e1t(ji-1,jj)
                  z05cx = un(ji-1,jj,jk) / e1t(ji-1,jj) * rdt
                  z05cx = min( z05cx, 1. )
         ...
         ! ... update ( ta, sa ) with radiative or climatological (t, s)
                  ta(ji,jj,jk) = ta(ji,jj,jk) * (1. - temsk(jj,jk)) +           & 
                                 ...
                                 * tebnd(jj,jk,nibm,nit ) + ztau * tfoe (jj,jk,1) ) &
                                 / (1. + z05cx)
                  sa(ji,jj,jk) = sa(ji,jj,jk) * (1. - temsk(jj,jk)) +           & 
                                 temsk(jj,jk) * ( ( 1. - z05cx - ztau )         &
            ...
            DO jk = 1, jpkm1
               DO jj = njw0, njw1
                  ta(ji,jj,jk) = ta(ji,jj,jk) * (1. - twmsk(jj,jk)) + &
                                 tfow(jj,jk,1)*twmsk(jj,jk)
                  sa(ji,jj,jk) = sa(ji,jj,jk) * (1. - twmsk(jj,jk)) + &
                                 sfow(jj,jk,1)*twmsk(jj,jk)
               END DO
            END DO
         END DO
      ...
      ! -----------------------------------------------------
         ...
         DO ji = fs_niw0, fs_niw1 ! Vector opt.
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1
!FD change for an advection characteristic + relaxation
!FD      ! ... i-phase speed ratio (from averaged of v_cxwbnd)
!FD               z05cx = (  0.5 * ( v_cxwbnd(jj,jk) + v_cxwbnd(jj-1,jk) ) ) / e1t(ji+1,jj)
                  z05cx = un(ji,jj,jk) / e1t(ji+1,jj) * rdt
                  z05cx = max( z05cx, -1. )
         ! ... z05cx > 0, inflow  zin=0, ztau=1    
      ...
      ! -------------------------------------------------------
         ...
         DO jj = fs_njn0+1, fs_njn1+1 ! Vector opt.
            DO jk = 1, jpkm1
               DO ji = 2, jpim1
!FD change for an advection characteristic + relaxation
!FD	 ! ... j-phase speed ratio (from averaged of vtnbnd)
!FD	 !	  (bounded by 1)
!FD		  z05cx = ( 0.5 * ( u_cynbnd(ji,jk) + u_cynbnd(ji-1,jk) ) ) / e2t(ji,jj-1)
                  z05cx = vn(ji,jj-1,jk) / e2t(ji,jj-1) * rdt
         	  z05cx = min( z05cx, 1. )
         ! ... z05cx=< 0, inflow  zin=0, ztau=1    
      ...
      ! -------------------------------------------------------
         ...
         DO jj = fs_njs0, fs_njs1  ! Vector opt.
            DO jk = 1, jpkm1
               DO ji = 2, jpim1
!FD change for an advection characteristic + relaxation
!FD      !... j-phase speed ratio (from averaged of u_cysbnd)
!FD      !       (bounded by 1)
!FD               z05cx = ( 0.5 * ( u_cysbnd(ji,jk) + u_cysbnd(ji-1,jk) ) ) / e2t(ji,jj+1)
                  z05cx = vn(ji,jj,jk) / e2t(ji,jj+1) * rdt
                  z05cx = max( z05cx, -1. )
         !... z05cx > 0, inflow  zin=0, ztau=1
         !          < 0, outflow zin=1, ztau=rtaus
                  zin = sign( 1., -1.* z05cx )
                  zin = 0.5*( zin + abs(zin) )
!FD               ztau = (1.-zin ) + zin * rtaus
