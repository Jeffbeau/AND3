...
CONTAINS
   ...
   SUBROUTINE obc_dom
      ...
      !! $Header: /home/opalod/NEMOCVSROOT/NEMO/OPA_SRC/OBC/obcdom.F90,v 1.5 2006/05/11 15:15:31 opalod Exp $ 
      ...
      IF( nbobc == 1 .OR. nbic == 0 ) CALL ctl_stop( ' obc_dom: No isolated coastlines gcfobc is set to zero' )
 ...
 9222 FORMAT(i4,41i3)
      ...
      IF( icheck /= 0 ) THEN
         WRITE(ctmp1,*) 'obc_dom : tmask and isolated coastlines mask are not equal', icheck
	 CALL ctl_stop( ctmp1 )
      END IF
      ...
      DO jnic = 1, nbobc-1
         IF( mnic(0,jnic) > jpnic ) THEN
            WRITE(ctmp1,*) 'obc_dom: isolated coastline ',jnic,' has ',ip,' grid-points > ',jpnic 
            CALL ctl_stop( ctmp1 )
         END IF
         IF( mnic(0,jnic) == 0 ) THEN
            WRITE(ctmp1,*) 'obc_dom: isolated coastline ',jnic,' has 0  grid-points verify coastlines file'
            CALL ctl_stop( ctmp1 )
         END IF
      END DO
