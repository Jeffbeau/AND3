...
CONTAINS
   ...
   SUBROUTINE obc_dyn ( kt )
      ...
      !!      else  (rigid lid case) , 
      !!                 this routine is called in dynnxt.F routine and updates ua, va. 
      !!
...
! FD                 ua(ji,jj,jk) = ua(ji,jj,jk) * ( 1. - uemsk(jj,jk) ) +          &
! FD                                uemsk(jj,jk) * (  ( 1. - z05cx - ztau )         &
! FD                                * uebnd(jj,jk,nib ,nitm) + 2.*z05cx               &
! FD                                * uebnd(jj,jk,nibm,nit ) + ztau * ufoe (jj,jk) )  &
! FD                                / (1. + z05cx)
! FD specifying OBC instead
                  ua(ji,jj,jk) = ua(ji,jj,jk) * (1.-uemsk(jj,jk)) + &
                                 uemsk(jj,jk)*ufoe(jj,jk)
               END DO
            END DO
...
# if defined key_dynspg_rl
         ...
         ! ... add the correct barotropic radiative velocity (calculated from bsfn) to the 
         !     baroclinc velocity uclie() to have the total velocity
         DO ji = nie0, nie1
            DO jk = 1, jpkm1
               DO jj = 1, jpj
                  ua(ji,jj,jk) = ua(ji,jj,jk) * ( 1. - uemsk(jj,jk) ) +                     &
                                 uemsk(jj,jk) * ( uclie(jj,jk) -  hur (ji,jj) / e2u (ji,jj) &
                                 * ( bsfn(ji,jj) - bsfn(ji,jj-1) ) )
               END DO
            END DO
         END DO
...
! FD                  va(ji,jj,jk) = va(ji,jj,jk) * (1. - vemsk(jj,jk) ) +          &
! FD                                 vemsk(jj,jk) * ( ( 1. - z05cx - ztau )         &
! FD                                 * vebnd(jj,jk,nib ,nitm) + 2.*z05cx              &
! FD                                 * vebnd(jj,jk,nibm,nit ) + ztau * vfoe(jj,jk) )  &
! FD                                 / (1. + z05cx)
! FD specifying OBC instead
                  va(ji,jj,jk) = va(ji,jj,jk) * (1.-vemsk(jj,jk)) + &
                                 vfoe(jj,jk)*vemsk(jj,jk)
...
! FD                  ua(ji,jj,jk) = ua(ji,jj,jk) * ( 1. - uwmsk(jj,jk) ) +          &
! FD                                 uwmsk(jj,jk) * ( ( 1. + z05cx - ztau )          &
! FD                                 * uwbnd(jj,jk,nib ,nitm) - 2.*z05cx               &
! FD                                 * uwbnd(jj,jk,nibm,nit ) + ztau  * ufow (jj,jk) ) &
! FD                                 / (1. - z05cx)
! FD specifying OBC instead
                  ua(ji,jj,jk) = ua(ji,jj,jk) * (1.-uwmsk(jj,jk)) + &
                                 uwmsk(jj,jk)*ufow(jj,jk)
...
# if defined key_dynspg_rl
         ...
         ! ... add the correct barotropic radiative velocity (calculated from bsfn)
         !     to the baroclinc velocity ucliw() to have the total velocity
         DO ji = niw0, niw1
            DO jk = 1, jpkm1
               DO jj = 1, jpj
                  ua(ji,jj,jk) = ua(ji,jj,jk) * ( 1. - uwmsk(jj,jk) ) +                    &
                                 uwmsk(jj,jk)*( ucliw(jj,jk) - hur (ji,jj) / e2u (ji,jj)   &
                                 * ( bsfn(ji,jj) - bsfn(ji,jj-1) ) )
               END DO
            END DO
         END DO
# endif
         ...
         !          =< 0, outflow zin=1, ztau=rtauw
                  ...
                  z05cx = z05cx * zin 
         ! ... update va with radiative or climatological velocity
! FD                  va(ji,jj,jk) = va(ji,jj,jk) * (1. - vwmsk(jj,jk) ) +          &
! FD                                 vwmsk(jj,jk) * ( ( 1. + z05cx - ztau )         &
! FD                                 * vwbnd(jj,jk,nib ,nitm) - 2.*z05cx              &
! FD                                 * vwbnd(jj,jk,nibm,nit ) + ztau * vfow (jj,jk) ) &
! FD                                 / (1. - z05cx)
! FD specifying OBC instead
                  va(ji,jj,jk) = va(ji,jj,jk) * (1.-vwmsk(jj,jk)) + &
                                 vfow(jj,jk)*vwmsk(jj,jk)
...
# endif
      ...
      ! ------------------------------------------------------
         ...
         ! ... for u, when inflow, ufon is prescribed
                  z05cx = z05cx * zin
         ! ... update un with radiative or climatological velocity
! FD                  ua(ji,jj,jk) = ua(ji,jj,jk) * (1.-unmsk(ji,jk)) +             &
! FD                                 unmsk(ji,jk) * ( ( 1. - z05cx - ztau )         &
! FD                                 * unbnd(ji,jk,nib ,nitm) + 2.*z05cx              &
! FD                                 * unbnd(ji,jk,nibm,nit ) + ztau * ufon (ji,jk) ) &
! FD                                 / (1. + z05cx)
! FD specifying OBC instead
                  ua(ji,jj,jk)= ua(ji,jj,jk) * (1.-unmsk(ji,jk)) + &
                                ufon(ji,jk)*unmsk(ji,jk)
         ...
         !              |         |
         !
         ! ... If rigidlid formulation:
         ! ... radiative conditions on the baroclinic part only + relaxation toward climatology 
         ! ... If free surface formulation:
         ! ... radiative conditions on the total part + relaxation toward climatology
         ...
         ! ... for inflow rtaunin is used for relaxation coefficient else rtaun
                  ...
                  z05cx = z05cx * zin
         ! ... update va with radiative or climatological velocity
! FD                  va(ji,jj,jk) = va(ji,jj,jk) * (1.-vnmsk(ji,jk)) +             &
! FD                                 vnmsk(ji,jk) * ( ( 1. - z05cx - ztau )         &
! FD                                 * vnbnd(ji,jk,nib ,nitm) + 2.*z05cx              &
! FD                                 * vnbnd(ji,jk,nibm,nit ) + ztau * vfon (ji,jk) ) &
! FD                                 / (1. + z05cx)
! FD specifying OBC instead
                  va(ji,jj,jk)= va(ji,jj,jk) * (1.-vnmsk(ji,jk)) + &
                                vfon(ji,jk)*vnmsk(ji,jk)
...
# if defined key_dynspg_rl
         ...
         ! ... add the correct barotropic radiative velocity (calculated from bsfn)
         !     to the baroclinc velocity vclin() to have the total velocity
         DO jj = njn0, njn1
            DO jk = 1, jpkm1
               DO ji = 1, jpi
                  va(ji,jj,jk) = va(ji,jj,jk) * (1.-vnmsk(ji,jk)) +                         &
                                 vnmsk(ji,jk) * ( vclin(ji,jk) +  hvr (ji,jj) / e1v (ji,jj) &
                                 * ( bsfn(ji,jj) - bsfn(ji-1,jj) ) )
               END DO
            END DO
         END DO
...
# endif
      ...
      ! ------------------------------------------------------
         ...
         !          =< 0, outflow zin=1, ztau=rtaus
                  ...
                  z05cx = z05cx * zin 
         ! ... update ua with radiative or climatological velocity
! FD                  ua(ji,jj,jk) = ua(ji,jj,jk) * (1.-usmsk(ji,jk)) +              &
! FD                                 usmsk(ji,jk) * (  ( 1. + z05cx - ztau )         &
! FD                                 * usbnd(ji,jk,nib ,nitm) - 2.*z05cx               &
! FD                                 * usbnd(ji,jk,nibm,nit ) + ztau * ufos (ji,jk) )  &
! FD                                 / (1. - z05cx)
! FD specifying OBC instead
                  ua(ji,jj,jk)= ua(ji,jj,jk) * (1.-usmsk(ji,jk)) + &
                                usmsk(ji,jk) * ufos(ji,jk)
         ...
         !         =< 0, outflow zin=1, ztau=rtaus
                  ...
                  z05cx = z05cx * zin
         ! ... update va with radiative or climatological velocity
! FD                  va(ji,jj,jk) = va(ji,jj,jk) * (1.-vsmsk(ji,jk)) +             &
! FD                                 vsmsk(ji,jk) * ( ( 1. + z05cx - ztau )         &
! FD                                 * vsbnd(ji,jk,nib ,nitm) - 2.*z05cx              &
! FD                                 * vsbnd(ji,jk,nibm,nit ) + ztau * vfos (ji,jk) ) &
! FD                                 / (1. - z05cx)
! FD specifying OBC instead
                  va(ji,jj,jk)= va(ji,jj,jk) * (1.-vsmsk(ji,jk)) + &
                                vsmsk(ji,jk) * vfos(ji,jk)
...
# if defined key_dynspg_rl 
         ...
         ! ... add the correct barotropic radiative velocity (calculated from bsfn)
         !     to the baroclinic velocity vclis() to have the total velocity
         DO jj = njs0, njs1
            DO jk = 1, jpkm1
               DO ji = 1, jpi
                  va(ji,jj,jk) = va(ji,jj,jk) * (1.-vsmsk(ji,jk)) +                         &
                                 vsmsk(ji,jk) * ( vclis(ji,jk) +  hvr (ji,jj) / e1v (ji,jj) &
                                 * ( bsfn(ji,jj) - bsfn(ji-1,jj) ) )
               END DO
            END DO
         END DO
