...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Header: /home/opalod/NEMOCVSROOT/NEMO/OPA_SRC/DOM/dommsk.F90,v 1.10 2006/03/21 08:25:42 opalod Exp $ 
...
!ylu
!!DB -- 2008.11.24 -- Is this the cause of my probs in SE/SW corners ???
!SPINUP Run ---> garbage ~10d
!      write(2200+narea,*)'DBG: not setting tmask in SE/SW corners to zero'
      tmask(mi0(1):mi1(2),mj0(1):mj1(2),1:jpk)=0.0
      tmask(mi0(196):mi1(197),mj0(1):mj1(2),1:jpk)=0.0
...
!      IF( cp_cfg == "orca" .AND. jp_cfg == 2 ) THEN
   ...
   SUBROUTINE dom_msk_nsa
      ...
      !!----------------------------------------------------------------------
      !! *Local declarations
      INTEGER  :: ji, jj, jk, ii, jl ! dummy loop indices
...
!                  IF( nprint == 1 ) WRITE(numout,*) ' west  : ', jk,inw, ji, jj
                  IF( nprint == 1 .AND. lwp) WRITE(numout,*) ' west  : ', jk,inw, ji, jj
...
!                  IF( nprint == 1 ) WRITE(numout,*) ' east  : ', jk, ine, ji, jj
                  IF( nprint == 1 .AND. lwp) WRITE(numout,*) ' east  : ', jk, ine, ji, jj
               ENDIF
            END DO
         END DO
         npcoa(1,jk) = inw
...
!                  IF( nprint == 1 ) WRITE(numout,*) ' south : ', jk, ins, ji, jj
                  IF( nprint == 1 .AND. lwp) WRITE(numout,*) ' south : ', jk, ins, ji, jj
               ENDIF
               ...
               IF( ABS(zaa-2.) <= 0.1 .AND. fmask(ji,jj,jk) == 0 ) THEN
                  ...
                  nicoa(inn,4,jk) = ji
                  njcoa(inn,4,jk) = jj
                  IF( nprint == 1 .AND. lwp) WRITE(numout,*) ' north : ', jk, inn, ji, jj
               ENDIF
            END DO
         END DO
         npcoa(3,jk) = ins
